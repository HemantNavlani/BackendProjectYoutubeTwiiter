...
const updateVideo =asyncHandler(async(req,res)=>{
    ...
    if (video.owner.toString() !== req.user?._id.toString()) throw new ApiError(400,"You can't update video only its owner can");
    ...
    if (!updatedVideo) throw new ApiError(500,"Failed to update video, try again");

    if (updatedVideo) deleteFromCloudinary(thumbnailToDelete);
    return res
    .status(200)
...
const deleteVideo = asyncHandler(async(req,res)=>{
    ...
    if (req.user?._id.toString() !== video.owner.toString()) throw new ApiError(400,"You can't delete, you are not the owner");
...
const toggelPublishStatus = asyncHandler(async(req,res)=>{
    ...
    const toggledVideoPublished = await Video.findByIdAndUpdate(
        videoId,
        {
            $set:{
    ...
    if (!toggledVideoPublished) throw new ApiError(500,"Could not toggle publish video");

    return res.status(200).json(new ApiResponse(200,toggledVideoPublished,"Video is published toggled successfully"))
